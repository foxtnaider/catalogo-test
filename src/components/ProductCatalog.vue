<script setup>
    ...
    const formatStock = (stock) => {
        // Convertir el valor a un número y formatearlo con dos decimales
        const stockNumber = parseFloat(stock);
        return Number.isInteger(stockNumber) 
            ? stockNumber.toString() 
            : stockNumber.toFixed(2).replace('.', ',');
    };
    ...
    const selectProduct = async (product_id , product_name) => {
        console.log(product_id);
        console.log(product_name);
        router.push({ name: 'ProducDetailView', params: { product_id, product_name } });
    };
</script>
    ...
    <div v-for="(product, index) in products" :key="index" class="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
        ...
        <div class="px-5 pb-5">
            ...
            <p class="text-gray-500 mb-2">
                {{ product.stock > 0 ? `${formatStock(product.stock)} in stock` : 'Out of Stock' }}
            </p>
            <div class="flex items-center justify-between">
                <span class="text-lg font-bold text-gray-900 dark:text-white">Ref: {{ formatPrice(product.precio, 'USD') }} </span>
